            ...
            {navItems.map((item, index) => (
              <div key={index} className="relative flex">
                ...
                {item?.children && activeDropdown === true && (
                  <div
                    className="absolute mt-6 shadow-xl rounded-md top-full -left-5  bg-center bg-no-repeat bg-[url('/Banner01.png')] bg-cover   z-50"
                    onMouseLeave={() => setActiveDropdown(false)}
                    ref={wrapperRef}
                  >
                    <div
                      className={`${
                        isScrolled ? "bg-white/96" : "bg-white/96"
                      }  max-w-6xl  p-2 rounded-md`}
                    >
                      {item?.children?.map((location, i) => (
                        ...
                        >
                          <div className="  text-black my-2">
                            <Link
                              href={location?.url}
                              className="text-black p-4 hover:text-primary text-lg font-normal "
                              role="menuitem"
                            >
                              {location.label}
                            </Link>
